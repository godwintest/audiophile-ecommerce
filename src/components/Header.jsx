...
export default function Header({ extraClass }) {
  ...
  const { cartData, clearCart, updateCartItemQuantity } =
    ...
    <>
      <header className={extraClass}>
        ...
        <nav className="uppercase">
          <ul
            className="flex primary-navigation ff-sanserif font-white"
        ...
        <div className="cart-toggle-parent">
          <button className="cart-toggle" onClick={handleCartClick}>
            ...
            <div
              className="cart-toggle-counter"
              style={
                cartData.length ? { display: "flex" } : { display: "none" }
              }
            >
              {cartData.length}
            </div>
          ...
          {isCartExpanded && (
            <div className="cart-container ff-sanserif">
              <div className="cart-header">
                ...
                <h2 className="fs-body" onClick={removeAll}>
                  Remove all
                </h2>
              </div>
              ...
              {cartData.length > 0 ? (
                cartData.map((item, index) => {
                  return (
                    <div
                      className="cart-item-container ff-sanserif"
                      key={index}
                    >
                      <div className="cart-item">
                        <img
                          src={
                            "../cart/image-" + item.currentProduct.slug + ".jpg"
                          }
                          alt="product image"
                        />
                        <div className="cart-item-text">
                          <h1 className="fs-body">
                            {
                              item.currentProduct.name.split(
                                item.currentProduct.category
                                  .charAt(0)
                                  .toUpperCase() +
                                  item.currentProduct.category.slice(1)
                              )[0]
                            }
                          </h1>
                          <h2 className="font-black" style={{ opacity: "0.5" }}>
                            ${addCommasToPrice(item.currentProduct.price)}
                          </h2>
                        </div>
                      </div>
                      {/* <ItemCounter /> */}
              ...
              <Link to="/checkout">
                <button className="btn-cart uppercase font-white fs-subtitle">
                  Checkout
                </button>
              </Link>
            </div>
