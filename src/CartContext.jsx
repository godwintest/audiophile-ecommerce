...
export const CartProvider = ({ children }) => {
  ...
  const updateCartData = (newData) => {
    setCartData((prevData) => {
      if (
        prevData.some(
          (item) => item.currentProduct.id === newData.currentProduct.id
        )
      ) {
        const updatedArray = prevData.map((item) => {
          if (item.currentProduct.id === newData.currentProduct.id) {
            return {
              ...item,
              itemQuantity: item.itemQuantity + newData.itemQuantity,
            };
          } else {
            return item;
          }
  ...
  return (
    <CartContext.Provider
      value={{ cartData, updateCartData, updateCartItemQuantity, clearCart }}
    >
      {children}
    </CartContext.Provider>
  );
